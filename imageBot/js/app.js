...
async function getData(maxPredictionClass) {
  $.ajax({
    url: 'https://demo-translate-zovivo.herokuapp.com/dish/get-by-name?name=' + maxPredictionClass.name.trim(),
    type: 'get',
    dataType: 'json',
    cache: false,
  ...
  if (restaurants.length > 0) {
    ...
    restaurants.map((restaurant) => {
      listImage += '<div class="col-lg-3 col-md-6 mb-4">'
        ...
        + '<div class="card-body">'
        + '<h4 class="card-title">'
        + '<a href="../restaurant/index.html?id=' + restaurant.id + '"> ' + restaurant.name + '</a>'
        + '</h4>'
